	...
	void handleMessage(UvpMessageT* pMsg)
	{
		if (pMsg->m_Code == UvpMessageT::MESSAGE_CODE::PROGRESS_REPORT)
		{
			...
			for (int i = 0; i < pReportProgressMsg->m_ProgressSize; i++)
			{
				...
				if (pReportProgressMsg->m_PackingPhase == uvpcore::UVP_PACKING_PHASE_CODE::PACKING)
				{
					...
					case uvpcore::UVP_PACKING_PHASE_CODE::PACKING:
						...
						break;
					case uvpcore::UVP_PACKING_PHASE_CODE::DONE:
...
// Create a Log Message class so we can print to the event log, Log System
class Log : public CLxLogMessage
{
public:
	Log() : CLxLogMessage(LXsLOG_LOGSYS) {}
	virtual ~Log() {}

	const char* GetFormat() { return "packit"; }
	const char* GetVersion() { return "0.2"; }
};
...
public:
	...
	// Variables to store command arguments,
	int orient;
	int stretch;

	// Make sure the command is disabled with not active layers
	bool enable()
	{
		int flags = 0;
		unsigned count;
	...
	void setup_args(CLxAttributeDesc &description) LXx_OVERRIDE
	{
		description.add("stretch", LXsTYPE_BOOLEAN);
		description.add("orient", LXsTYPE_BOOLEAN);
	}
	...
	void execute() LXx_OVERRIDE
	{
		...
		// When stretch is set to true, the packer will not scale islands during packing.
		// If UV islands can’t fit into the packing box, the NO_SPACE code 
		// will be returned by the operation.

		// Reading argument "stretch" to match existing Modo Packer,
		cmd_read_arg("stretch", stretch);
